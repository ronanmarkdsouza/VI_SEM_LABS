...
def main():
    ...
    path_choice = input("Enter a search method from BFS or DFS: ")

    if path_choice == 'BFS':
        path_bfs = shortest_path_bfs(source, target)
        ...
        if path_bfs is None:
            ...
            degrees = len(path_bfs)
            print(f"{degrees} degrees of separation.")
            path_bfs = [(None, source)] + path_bfs
            for i in range(degrees):
                person1 = people[path_bfs[i][1]]["name"]
                person2 = people[path_bfs[i + 1][1]]["name"]
                movie = movies[path_bfs[i + 1][0]]["title"]
                print(f"{i + 1}: {person1} and {person2} starred in {movie}")
    
    elif path_choice == 'DFS':
        path_dfs = shortest_path_dfs(source, target)
        ...
        if path_dfs is None:
            ...
            degrees = len(path_dfs)
            print(f"{degrees} degrees of separation.")
            path_dfs = [(None, source)] + path_dfs
            for i in range(degrees):
                person1 = people[path_dfs[i][1]]["name"]
                person2 = people[path_dfs[i + 1][1]]["name"]
                movie = movies[path_dfs[i + 1][0]]["title"]
                print(f"{i + 1}: {person1} and {person2} starred in {movie}")
...
def shortest_path_dfs(source, target):
    ...
    # Initialise a StackFrontier for DFS, with the starting actor ID:
    start = Node(source, None, None)
